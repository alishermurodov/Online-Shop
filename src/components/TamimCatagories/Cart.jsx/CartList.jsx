...
const CartList = () => {
  ...
  return (
    <div className=' pt-[50px]'>
      ...
      <div className='grid grid-cols-4 gap-[50px] mt-[100px] items-center w-[80%] mx-auto max:flex-wrap justify-center ml-[16%] max3:ml-[0] dark:text-white' >
        <h1>Product</h1>
        <h1>Price</h1>
        <h1>Quantity</h1>
        <h1>Subtotal</h1>
        ...
        {products.map((product, index) => (
          <React.Fragment key={index}>
            <div className='flex gap-[25px] items-center max:flex-wrap dark:text-white' >
              <img src={product.image} alt="" className='self-start object-cover' />
              <h1>{product.name}</h1>
            </div>
            <h1 className='dark:text-white'>${product.price}</h1>
            <input type="number" value={product.quantity} onChange={(e) => handleQuantityChange(index, e)} className='w-[72px] h-[42px] border-[1.5px] border-solid border-[#00000066] dark:bg-[#272727] pl-[25px] rounded-[4px]' />
            <h1>${product.price * product.quantity}</h1> {/* Multiply price by quantity for subtotal */}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className='flex justify-around mt-[100px] max:flex-wrap gap-[10px] dark:text-white '>
        <button className='w-[218px] h-[56px] rounded-[4px] border-[1px] border-solid border-[#00000080] dark:border-[white]'>Return To Shop</button>
      ...
      <div className='flex justify-end mr-[340px] mt-[-100px] max2:justify-center max:m-[0] max2:pl-[20px] max2:mt-[100px] dark:text-white p-[8%]'>
